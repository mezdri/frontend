...
export const fetchPeajes = (id: string, rut: string) =>
  makeHttpRequest('getPeajes', id, rut).then(getResponseData);

export const fetchDias = () =>
  makeHttpRequestNormalized('getDias', {}).then(getResponseData);

export const fetchTarifas = (id_peaje: number) =>
  makeHttpRequestNormalized('getTarifas', { id_peaje: id_peaje }).then(
    getResponseData
  );
