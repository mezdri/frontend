...
export default class CamaraEnVivo extends Vue {
  ...
  async handleList(item: any) {
    ...
    var self = this;
    const cancelCheckbox = (text: string, time: number) => {
      self.$snotify.error(text);
      setTimeout(() => {
        item.checkbox = false;
      }, time);
      return;
    ...
    if (this.type_cam_old && item.checkbox) {
      if (item.type == 'parent') {
        if (this.getLengthParent(item) + this.listCam.length > 5) {
          this.$snotify.error(
            'No se puede tener más de 5 canales en visualización a la vez para este tipo de cámaras'
          );
          setTimeout(() => {
            item.checkbox = false;
          }, 300);
  ...
  valideLimitSize(item: any): boolean {
    var currentSize: number = this.listCam.length;
    if (item.type == 'parent') {
      if (item.checkbox) {
        currentSize = currentSize + item.children.length;
      }
    } else if (item.type == 'children') {
      if (item.checkbox) {
        currentSize += 1;
  ...
  resetMap() {
    ...
    this.chargeCoords();
    var self = this;
    // this.timerUpdateCoordinates = setInterval(function() {
    //   self.chargeCoords();
    // }, 30000);
  }
  ...
  setCurrentPatent() {
    var lastPatent: any = {};
    for (var item of this.treeViewList) {
      if (item.type == 'parent') {
        lastPatent = item;
      }
      //console.log(item);
    }
    this.handleCam(lastPatent);
  }
