<template>
  ...
  >
    ...
    <v-list v-show="mostrarMenuAcciones">
      ...
      <v-list-tile style="height: 40px" v-if="btnCancelar">
        <v-list-tile-avatar style="min-width: 35px">
      ...
      <v-list-tile style="height: 40px" v-if="btnAnular">
        <v-list-tile-avatar style="min-width: 35px">
          <v-btn small icon @click="handleItemClick(Accion.Anular)">
            <v-icon small color="#526DA5">lock</v-icon>
          </v-btn>
        </v-list-tile-avatar>
        <v-list-tile-title
          style="cursor: pointer; font-size: 13px; color: rgba(0, 0, 0, 0.7)"
          @click="handleItemClick(Accion.Anular)"
          >Anular</v-list-tile-title
        >
      </v-list-tile>
    </v-list>
  </v-menu>
...
export default Vue.extend({
  ...
  methods: {
    ...
    chequearPermisos() {
      ...
      if (this.params.codigoAccionesMenuItem.length > 0) {
        ...
        const accionAnular = this.params.usuario.permisosUsuario.find(
          x =>
            x.moduloCodigo === this.params.funcionalidad &&
            x.accionCodigo === CodigoAccion.Anular &&
            x.clienteId === config.getClienteSesion()
        );
        if (accionAnular !== undefined && accionAnular !== null) {
          this.btnAnular = true;
        } else {
  ...
  beforeMount() {
    ...
    if (
      this.params.funcionalidad === Funcionalidades.VerFacturacion ||
      this.params.funcionalidad === Funcionalidades.MovimientoDispositivos
    ) {
